...
const uint8_t ardubus_pca9635RGBJBOL_boards[] = ARDUBUS_PCA9635RGBJBOL_BOARDS;
...
{
    I2c.begin();
    for (uint8_t i=0; i < sizeof(ardubus_pca9635RGBJBOL_boards); i++)
    {
...
inline void ardubus_pca9635RGBJBOL_process_command(char *incoming_command)
{
    ...
    {
        case 0x4A: // ASCII "J" (J<indexuint8_t><ledbyte><value>) //Note that the indexbyte is index of the pca9635RGBJBOLs-array, not pin number, ledbyte is the number of the led on the board
